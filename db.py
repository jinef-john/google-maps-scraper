...
class Database:
    ...
    def upsert_place(self, place):
        ...
        self._conn().execute(
            ...
            (
                ...
                json.dumps(place.categories), hours,
    ...
    def mark_reviews_fetched(self, place_id, cursor=""):
        self._conn().execute(
            "UPDATE places SET reviews_fetched = 1, reviews_cursor = ? WHERE place_id = ?",
            (cursor, place_id),
        )
        self._conn().commit()
    ...
    def get_place_cursor(self, place_id):
        row = self._conn().execute(
            "SELECT reviews_cursor FROM places WHERE place_id = ?", (place_id,)
        ).fetchone()
        return row[0] if row else ""

    def get_pending_places(self):
        rows = self._conn().execute(
            "SELECT place_id, name, reviews_cursor FROM places WHERE reviews_fetched = 0"
        ).fetchall()
        return [{"place_id": r[0], "name": r[1], "cursor": r[2] or ""} for r in rows]
    ...
    def get_pending_job_places(self, job_id):
        rows = self._conn().execute(
            """SELECT jp.place_id, jp.reviews_cursor, p.name
               FROM job_places jp
        ...
        return [{"place_id": r[0], "cursor": r[1] or "", "name": r[2] or ""} for r in rows]
